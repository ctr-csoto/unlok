...
class Media{
      ...
      try {
        if(file.mimetype.split("/")[0] == "video"){
          ...
          if(user.type == 'unlokplus'){
            let { logo, pos_X, pos_Y } = await this.formatLogoWatermaker(size_w_img, size_h_img);
          ...
          image.print(
            ...
            upload_arr.push(this.uploadToS3Buffer(user.id, user.type, file.originalname, mi_buffer));
  ...
  async formatLogoWatermaker(size_w, size_h, get_path = false){
    try {
      const logo = await Jimp.read(path.resolve(__dirname, "../", "../", "../", "resources", "images", "unlok_50_red.png"));
          let percent = 100;
          let logo_h = 50;
          if(size_w < WIDTH_IMG){
          ...
          let url_img = "";
          if(get_path){
            url_img = `${new Date().getTime()}_${(Math.floor(Math.random()*100))}_prueba_img.png`
            logo.write(url_img); 
          }
